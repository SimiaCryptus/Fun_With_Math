...
export class RumorNetwork {
  ...
  observe(npc, ctx) {
    ...
    if (h.kind === 'FIRE' || h.kind === 'SMOKE') {
      for (const exit of this.state.grid.exits()) {
        if (exit.coord.z === h.z && Grid3D.distance(exit.coord, h) <= 3)
          this.originate(npc, { claimType: 'EXIT_BLOCKED', exitKey: exit.key, veracity: true });
      }
    } else if (h.kind === 'THREAT') {
      this.originate(npc, {
        claimType: 'THREAT_LOCATION',
        threatPos: { x: h.x, y: h.y, z: h.z },
        veracity: true,
      });
    }
  }
  ...
  originate(npc, claim) {
    ...
    const same = rumors.find(
      (r) => r.claimType === claim.claimType && r.exitKey === claim.exitKey && r.holders.has(npc.id)
    );
    if (same) return same;
    const rumor = {
      id: rumors.length + 1,
      ...
      confidenceScore: 0.9,
      distortionFactor: 0,
      holders: new Set([npc.id]),
      originTurn: this.state.meta.turnNumber,
      origin: npc.id,
    };
    rumors.push(rumor);
    ...
    this.state.log({
      ...
      npcId: npc.id,
      claimType: rumor.claimType,
      exitKey: rumor.exitKey || null,
    ...
    for (const rumor of [...s.social.rumors]) {
      for (const speakerId of [...rumor.holders]) {
        ...
        for (const listener of npcs) {
          if (
            listener.id === speakerId ||
            heardThisTurn.has(listener.id) ||
            rumor.holders.has(listener.id)
          )
            continue;
          if (
            listener.position.z !== speaker.position.z ||
            Grid3D.distance(listener.position, speaker.position) > ACOUSTIC_RANGE
          )
            continue;
          heardThisTurn.add(listener.id);
          const chance =
            clamp(listener.derived.stress * 0.6 + (1 - speaker.traits.trust) * 0.4) *
            (1 - s.social.groupCohesion * 0.5);
          if (s.random() < chance) this._distort(rumor, speaker, listener);
          else {
            rumor.holders.add(listener.id);
            this._adopt(listener, rumor);
          }
        }
      }
    }
  ...
  _distort(rumor, speaker, listener) {
    ...
    if (rumor.claimType === 'EXIT_BLOCKED') {
      const others = s.grid
        .exits()
        .filter((e) => e.key !== rumor.exitKey && e.coord.z === listener.position.z);
      if (!others.length) {
        rumor.holders.add(listener.id);
        this._adopt(listener, rumor);
        return;
      ...
      others.sort(
        (a, b) =>
          Grid3D.distance(a.coord, listener.position) - Grid3D.distance(b.coord, listener.position)
      );
      const target = others[0];
      distorted = {
        claimType: 'EXIT_BLOCKED',
        exitKey: target.key,
        veracity: this.isExitBlocked(target.key),
      };
    } else {
      ...
      distorted = {
        claimType: 'THREAT_LOCATION',
        threatPos: { x: dims.x - 1 - p.x, y: p.y, z: p.z },
        veracity: false,
      };
    }
    const nr = {
      id: s.social.rumors.length + 1,
      ...distorted,
      confidenceScore: rumor.confidenceScore * 0.75,
      distortionFactor: rumor.distortionFactor + 1,
      holders: new Set([listener.id]),
      originTurn: s.meta.turnNumber,
      origin: listener.id,
      parentId: rumor.id,
    };
    ...
    s.log({
      type: 'RUMOR_DISTORTED',
      speakerId: speaker.id,
      listenerId: listener.id,
      claimType: nr.claimType,
      fromExit: rumor.exitKey || null,
      toExit: nr.exitKey || null,
      veracity: nr.veracity,
    });
    this.bus.emit('RUMOR_DISTORTED', {
  ...
  clearFalseBeliefs(minTrust) {
    ...
    for (const npc of this.state.npcs.values()) {
      ...
      for (const exitKey of [...npc.beliefs.blockedExits]) {
        if (!this.isExitBlocked(exitKey)) {
          npc.beliefs.blockedExits.delete(exitKey);
          cleared++;
        }
      }
    }
