...
export class NPCManager {
  ...
  processTick() {
    ...
    for (const npc of npcs) {
      ...
      let action;
      if (npc.freezeTurns > 0) {
        npc.freezeTurns -= 1;
        action = { type: 'WAIT', reason: 'frozen' };
      ...
      const key = NPCSpine.selectDiagnosticDialog(npc, {
        ...ctx,
        lastAction: action.type,
        hasSocialFriction: action.type === 'ESCALATE_CONFLICT',
      });
      const stale = s.meta.turnNumber - npc.lastDialogTurn >= 3;
      if (key !== npc.lastDialogKey || stale) {
        if (!(key === 'STATUS_IDLE' && !stale))
          this.dialog.utter(
            npc,
            npc.lastDialogKey === 'RUMOR' || npc.lastDialogKey === 'RUMOR_THREAT'
              ? npc.lastDialogKey
              : key
          );
  ...
  buildContext(npc) {
    ...
    const points = [];
    for (const key of hazards.fireCells.keys()) {
      const c = parseKey(key);
      if (c.z === pos.z) points.push({ ...c, kind: 'FIRE' });
    }
    for (const [key, sm] of hazards.smokeCells)
      if (sm.density > 0.5) {
        const c = parseKey(key);
        if (c.z === pos.z) points.push({ ...c, kind: 'SMOKE' });
      }
    for (const t of hazards.threatEntities)
      if (!t.neutralized && t.position.z === pos.z) points.push({ ...t.position, kind: 'THREAT' });
    for (const tile of grid.tiles.values())
      if (tile.debris && tile.coord.z === pos.z) points.push({ ...tile.coord, kind: 'DEBRIS' });

    let nearest = null,
      nearestDist = 99;
    for (const h of points) {
      const d = Grid3D.distance(pos, h);
      if (d < nearestDist) {
        nearestDist = d;
        nearest = h;
      }
    ...
    for (const t of hazards.threatEntities) {
      if (
        !t.neutralized &&
        t.position.z === pos.z &&
        Grid3D.distance(pos, t.position) <= 12 &&
        hasLineOfSight(s, pos, t.position)
      ) {
        threatVisible = true;
        npc.beliefs.lastKnownThreat = { ...t.position };
      }
    }
    ...
    const playerVisible =
      ...
      playerDist <= 8 &&
      hasLineOfSight(s, pos, player.position);
    const nearbyAgents = [...s.npcs.values()].filter(
      (o) =>
        o.id !== npc.id &&
        o.status === 'ACTIVE' &&
        !o.carriedBy &&
        o.position.z === pos.z &&
        Grid3D.distance(pos, o.position) <= 3
    );
    const smokeHere = smokeDensityAt(s, keyOf(pos));
    const costFn = this.costFnFor(npc);
    const nearestStandardExit = findNearest(
      grid,
      pos,
      (t) =>
        t.type === 'EXIT' && !npc.beliefs.blockedExits.has(t.key) && !hazards.fireCells.has(t.key),
      { agent: { canOpenDoors: true }, costFn }
    );
  ...
  execute(npc, action, ctx) {
    ...
    switch (action.type) {
      ...
      case 'ESCALATE_CONFLICT': {
        ...
        if (Grid3D.distance(npc.position, target.position) <= 1.5)
          EscalationModel.bully(this.state, this.bus, npc, target);
      ...
      case 'FLEE_FROM':
        this.flee(npc, action.threatOrigin || ctx.nearestHazardLocation, speed);
        return;
      case 'FOLLOW_PLAYER':
        this.moveToward(npc, this.state.player.position, speed, 1);
        return;
      case 'SHELTER':
        this.shelter(npc, ctx, speed);
        return;
      case 'GATHER_BELONGINGS':
        this.state.log({ type: 'GREED_DELAY', npcId: npc.id, name: npc.name });
        return;
  ...
  moveAlong(npc, path, steps, opts = {}) {
    ...
    for (let i = 0; i < steps && i < path.length; i++) {
      const tile = path[i];
      if (s.hazards.fireCells.has(tile.key) || tile.debris) {
        npc.blockedTurns++;
        return false;
      }
      if (tile.type !== 'EXIT' && this._occupied(tile.key, npc)) {
        npc.blockedTurns++;
        return false;
      }
      if (tile.type === 'DOOR' && !tile.doorState.isOpen) {
        if (tile.doorState.isLocked || tile.doorState.isBarricaded) {
          npc.blockedTurns++;
          return false;
        }
      ...
      if (
        opts.closeBehind &&
        prev.type === 'DOOR' &&
        prev.doorState.isOpen &&
        !this._occupied(prev.key, null)
      )
        this._setDoor(prev, false, npc);
      if (npc.derived.stress > 0.6) pushNoise(s, npc.position, 5, npc.id);
      if (tile.type === 'EXIT') {
        evacuateNPC(s, this.bus, npc, 'reached exit');
        return true;
      }
      prev = tile;
    }
  ...
  moveToward(npc, target, steps, stopDistance = 0) {
    const path = findPath(this.state.grid, npc.position, target, {
      agent: { canOpenDoors: true },
      costFn: this.costFnFor(npc),
    });
    if (!path) {
      npc.blockedTurns++;
      return false;
    ...
    const trimmed =
      stopDistance > 0 ? path.slice(0, Math.max(0, path.length - stopDistance)) : path;
  ...
  flee(npc, origin, speed) {
    ...
    for (let step = 0; step < speed; step++) {
      ...
      for (const n of s.grid.neighbors(here.coord)) {
        ...
        let score =
          Grid3D.distance(t.coord, origin) -
          4 * smokeDensityAt(s, t.key) +
          (t.type === 'EXIT' ? 6 : 0) -
          (t.temperature > 100 ? 3 : 0);
      ...
      if (best.type === 'EXIT') {
        evacuateNPC(s, this.bus, npc, 'fled to exit');
        return;
      }
    }
  }
  ...
  wander(npc) {
    const s = this.state;
    const here = s.grid.getByKey(keyOf(npc.position));
    const options = s.grid
      .neighbors(here.coord, { diagonal: false })
      .map((n) => n.tile)
      .filter(
        (t) =>
          isTilePassable(t, { canOpenDoors: false }) &&
          !s.hazards.fireCells.has(t.key) &&
          !this._occupied(t.key, npc)
      );
  ...
  shelter(npc, ctx, speed) {
    ...
    for (const n of s.grid.neighbors(here.coord, { diagonal: false })) {
      ...
      if (
        t.type === 'DOOR' &&
        t.doorState.isOpen &&
        npc.traits.trust > 0.4 &&
        !this._occupied(t.key, null) &&
        !s.threatAt(t.key)
      ) {
    ...
    if (
      here.type === 'DOOR' ||
      this._nearWindow(here) ||
      this._adjacentToDoor(here) ||
      ctx.threatVisible
    ) {
      const found = findNearest(
        ...
        (t) =>
          ...
          t.key !== here.key &&
          !this._nearWindow(t) &&
          !this._adjacentToDoor(t) &&
          !this._occupied(t.key, npc),
        { agent: { canOpenDoors: false }, costFn: this.costFnFor(npc) }
      );
  ...
  applyDrift(npc, ctx) {
    ...
    t.fear = clamp01(
      t.fear +
        0.08 * prox +
        ctx.smokeHere * 0.1 -
        (ctx.hazardVisible || ctx.threatVisible ? 0 : 0.03)
    );
  ...
  _setDoor(tile, open, npc) {
    ...
    const rec = this.state.log({
      type: open ? 'DOOR_OPENED' : 'DOOR_CLOSED',
      key: tile.key,
      actor: npc.id,
    });
  ...
  _occupied(key, self) {
    ...
    for (const o of s.npcs.values()) {
      if (
        o !== self &&
        !o.carriedBy &&
        (o.status === 'ACTIVE' || o.status === 'INCAPACITATED') &&
        keyOf(o.position) === key
      )
        return true;
    }
    return false;
  ...
  _nearWindow(tile) {
    return this.state.grid
      .neighbors(tile.coord, { vertical: false })
      .some((n) => n.tile.type === 'WINDOW');
  }

  _adjacentToDoor(tile) {
    return this.state.grid
      .neighbors(tile.coord, { diagonal: false, vertical: false })
      .some((n) => n.tile.type === 'DOOR');
  }
}
