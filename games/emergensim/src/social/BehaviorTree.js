...
export class BehaviorTree {
  static arbitrate(npc, ctx, mode = 'EVACUATE') {
    ...
    if (d.stress > 0.85 && d.confidence < 0.2)
      return { type: 'FREEZE', duration: 1, reason: 'Paralyzing panic' };
  ...
  static _bullyTarget(npc, ctx) {
    ...
    return (
      ctx.nearbyAgents.find((o) => o.socialRank < npc.socialRank && o.traits.trust < 0.4) || null
    );
  }
  ...
  static _evacuate(npc, ctx) {
    ...
    if (target)
      return { type: 'ESCALATE_CONFLICT', targetId: target.id, conflictType: 'BULLY_INTIMIDATE' };
    // 2b. Follow a trusted leader
    if (ctx.playerVisible && t.trust > 0.65 && d.stress < 0.85 && ctx.playerDist > 1.5)
    ...
    // 4. EVASION & FLIGHT
    if (d.stress > 0.5 && ctx.nearestHazardLocation)
      return { type: 'FLEE_FROM', threatOrigin: ctx.nearestHazardLocation };
    // 4b. Greed delay
  ...
  static _lockdown(npc, ctx) {
    const d = npc.derived,
      t = npc.traits;
    if (ctx.threatVisible && ctx.lastKnownThreat)
      return { type: 'FLEE_FROM', threatOrigin: ctx.lastKnownThreat };
    if (ctx.playerVisible && t.trust > 0.6 && d.stress < 0.8 && ctx.playerDist > 1.5)
      return { type: 'FOLLOW_PLAYER' };
    if (t.cohesion > 0.5 && ctx.activeAlarm && d.stress < 0.85) return { type: 'SHELTER' };
    if (d.stress > 0.6 && ctx.lastKnownThreat)
      return { type: 'FLEE_FROM', threatOrigin: ctx.lastKnownThreat };
    const target = BehaviorTree._bullyTarget(npc, ctx);
    if (target)
      return { type: 'ESCALATE_CONFLICT', targetId: target.id, conflictType: 'BULLY_INTIMIDATE' };
  ...
  static _shelter(npc, ctx) {
    ...
    if (ctx.playerVisible && t.trust > 0.6 && ctx.playerDist > 1.5)
      return { type: 'FOLLOW_PLAYER' };
    if (d.stress > 0.5 && ctx.nearestHazardLocation)
      return { type: 'FLEE_FROM', threatOrigin: ctx.nearestHazardLocation };
    return { type: 'SEEK_INFORMATION' };
