...
export function findPath(
  ...
  start,
  goal,
  { costFn = null, agent = {}, passable = null, maxNodes = 6000 } = {}
) {
...
export function findNearest(
  ...
  start,
  predicate,
  { costFn = null, agent = {}, maxNodes = 5000 } = {}
) {
  const startKey = keyOf(start);
  ...
  while (heap.size) {
    ...
    closed.add(cur.key);
    const tile = grid.getByKey(cur.key);
    if (cur.key !== startKey && predicate(tile))
      return { tile, path: reconstruct(came, cur.key, startKey, grid), cost: dist.get(cur.key) };
    if (closed.size > maxNodes) break;
    for (const n of grid.neighbors(tile.coord)) {
      const nk = n.tile.key;
