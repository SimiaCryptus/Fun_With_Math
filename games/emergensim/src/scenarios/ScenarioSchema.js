...
export const SCENARIO_JSON_SCHEMA = {
  ...
  properties: {
    ...
    seed: { type: 'integer' },
    responderEtaTurns: { type: 'integer', minimum: 1 },
    maxTurns: { type: 'integer', minimum: 1 },
    groupCohesion: { type: 'number', minimum: 0, maximum: 1 },
    ...
    winConditions: {
      ...
      items: {
        ...
        properties: { type: { type: 'string', enum: WIN_TYPES }, targetValue: { type: 'number' } },
      },
    },
    events: {
      type: 'array',
...
export function inferGridSize(layout) {
  ...
  for (const d of layout) {
    gs.x = Math.max(gs.x, d.x + 1);
    gs.y = Math.max(gs.y, d.y + 1);
    gs.z = Math.max(gs.z, (d.z ?? 0) + 1);
  }
  return gs;
}
...
export function validateScenario(s) {
  ...
  if (!(isInt(s.responderEtaTurns) && s.responderEtaTurns > 0))
    err('responderEtaTurns must be a positive integer');
  if (!(isInt(s.maxTurns) && s.maxTurns > 0)) err('maxTurns must be a positive integer');
  if (typeof s.groupCohesion === 'number' && (s.groupCohesion < 0 || s.groupCohesion > 1))
    err('groupCohesion must be in [0,1]');
  ...
  if (!Array.isArray(s.mapLayout) || !s.mapLayout.length) {
    err('mapLayout must be a non-empty array (or provide `floors`)');
    return errors;
  }
  ...
  s.mapLayout.forEach((d, i) => {
    if (!hasIntXYZ(d)) {
      err(`mapLayout[${i}] has non-integer coordinates`);
      return;
    }
    if (gsOk && (d.x < 0 || d.y < 0 || d.z < 0 || d.x >= gs.x || d.y >= gs.y || d.z >= gs.z))
      err(`mapLayout[${i}] (${d.x},${d.y},${d.z}) lies outside gridSize`);
    if (!TILE_TYPES.includes(d.type)) err(`mapLayout[${i}] has unknown type "${d.type}"`);
    if (d.material && !MATERIALS[d.material])
      err(`mapLayout[${i}] has unknown material "${d.material}"`);
    if (
      d.flammability !== undefined &&
      (typeof d.flammability !== 'number' || d.flammability < 0 || d.flammability > 1)
    )
      err(`mapLayout[${i}] flammability must be in [0,1]`);
  ...
  for (const t of tiles.values()) {
    ...
    if (gs && gs.z > 1 && !(up && up.type === 'STAIR') && !(down && down.type === 'STAIR'))
      err(`STAIR at ${t.key} has no STAIR directly above or below`);
  }

  const tileAt = (p) => (hasIntXYZ(p) ? tiles.get(coordKey(p.x, p.y, p.z)) || null : null);
  const checkPos = (p, label, agentOpts = null) => {
    if (!hasIntXYZ(p)) {
      err(`${label}: position must have integer x, y, z`);
      return null;
    }
    const t = tileAt(p);
    if (!t) {
      err(`${label}: no tile at ${keyOf(p)}`);
      return null;
    }
    if (agentOpts && !isTilePassable(t, agentOpts))
      err(`${label}: tile ${t.key} (${t.type}) is not a valid standing position`);
    return t;
  };
  ...
  if (!s.player || typeof s.player !== 'object') err('player is required');
    ...
    if (s.player.inventory !== undefined && !Array.isArray(s.player.inventory))
      err('player.inventory must be an array');
  }
  ...
  if (!Array.isArray(s.initialHazards)) err('initialHazards must be an array');
    ...
    s.initialHazards.forEach((h, i) => {
      const label = `initialHazards[${i}]`;
      if (!h || !HAZARD_TYPES.includes(h.type)) {
        err(`${label}: type must be one of ${HAZARD_TYPES.join(' | ')}`);
        return;
      }
      if (
        h.intensity !== undefined &&
        (typeof h.intensity !== 'number' || h.intensity < 0 || h.intensity > 1)
      )
        err(`${label}: intensity must be in [0,1]`);
      if (h.type === 'THREAT') {
        checkPos(h.position, label, { canOpenDoors: true, canForce: true });
        occupy(h.position, label);
        (h.patrolRoute || []).forEach((p, j) =>
          checkPos(p, `${label}.patrolRoute[${j}]`, { canOpenDoors: true, canForce: true })
        );
        if (h.facing && (typeof h.facing.dx !== 'number' || typeof h.facing.dy !== 'number'))
          err(`${label}: facing must be { dx, dy }`);
      } else {
        const t = checkPos(h.position, label);
        if (t && h.type === 'FIRE' && t.material.flammability <= 0 && t.type !== 'CONTAINMENT')
          err(`${label}: FIRE placed on non-flammable ${t.type} at ${t.key}`);
      }
    });

  // ---- objectives
  if (!Array.isArray(s.winConditions) || !s.winConditions.length)
    err('winConditions must be a non-empty array');
  else
    s.winConditions.forEach((w, i) => {
      if (!w || !WIN_TYPES.includes(w.type)) {
        err(`winConditions[${i}]: type must be one of ${WIN_TYPES.join(' | ')}`);
        return;
      }
      if (typeof w.targetValue !== 'number')
        err(`winConditions[${i}]: targetValue must be a number`);
      else if (w.type === 'EVACUATE_MINIMUM_PERCENT' && (w.targetValue < 0 || w.targetValue > 1))
        err(`winConditions[${i}]: EVACUATE_MINIMUM_PERCENT targetValue is a fraction in [0,1]`);
      else if (w.targetValue < 0) err(`winConditions[${i}]: targetValue must be >= 0`);
    });
  const needsExit =
    s.mode === 'EVACUATE' ||
    (s.winConditions || []).some((w) => w?.type === 'EVACUATE_MINIMUM_PERCENT');
  if (needsExit && ![...tiles.values()].some((t) => t.type === 'EXIT'))
    err('an EVACUATE scenario needs at least one EXIT tile');
  ...
  if (s.events !== undefined && !Array.isArray(s.events)) err('events must be an array');
    ...
    (s.events || []).forEach((ev, i) => {
      ...
      if (!ev || !EVENT_TYPES.includes(ev.type)) {
        err(`${label}: type must be one of ${EVENT_TYPES.join(' | ')}`);
        return;
      }
      ...
      if (
        ...
        (typeof ev.magnitude !== 'number' || ev.magnitude < 0 || ev.magnitude > 1)
      )
        err(`${label}: magnitude must be in [0,1]`);
      if ((ev.type === 'FIRE' || ev.type === 'SMOKE') && !checkPos(ev.position, label)) {
        /* reported by checkPos */
      }
    });
...
export function normalizeScenario(def) {
  ...
  if (!Array.isArray(s.mapLayout) || !s.mapLayout.length) {
    if (!Array.isArray(s.floors))
      throw new Error(`Scenario "${s.id}": provide either mapLayout or floors`);
  ...
  s.player = {
    inventory: ['BARRICADE_KIT'],
    ...(s.player || {}),
    position: withZ(s.player?.position),
  };
  ...
  s.initialHazards = (s.initialHazards || []).map((h) => ({
    ...
    ...(h.type === 'THREAT'
      ? { patrolRoute: (h.patrolRoute || []).map(withZ), facing: h.facing || { dx: 1, dy: 0 } }
      : {}),
  }));
  s.winConditions = s.winConditions || [];
  s.events = [...(s.events || [])]
    .map((e) => (e && e.position ? { ...e, position: withZ(e.position) } : e))
