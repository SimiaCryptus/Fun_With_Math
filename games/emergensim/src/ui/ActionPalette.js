...
const TOOLS = [
  {
    ...
    label: 'Move',
    key: 'M',
    hint: 'Click a tile to walk there — closed doors on the route are opened (1 AP each)',
  },
...
export class ActionPalette {
  ...
  _bind() {
    ...
    const add = (target, type, fn) => {
      ...
      this._dom.push(() => target.removeEventListener(type, fn));
    };
    add(window, 'keydown', (e) => this._onKey(e));
    add(document.getElementById('btn-undo'), 'click', () => this.undo());
    add(document.getElementById('btn-autopsy'), 'click', () =>
      this.bus.emit('AUTOPSY_REQUESTED', {})
    );
  }
  ...
  inputOk() {
    ...
    return (
      ...
      !s.outcome &&
      s.player.physicalState.status === 'ACTIVE'
    );
  }

  dispatch(action) {
    this.bus.emit('PLAYER_ACTION_REQUESTED', action);
  }
  hint(text, kind = '') {
    this.bus.emit('HUD_HINT', { text, kind });
  }
  ...
  render() {
    ...
    const end = document.createElement('button');
      ...
      this._clearPath();
      this.bus.emit('STEP_TURN_REQUESTED', {});
    });
  ...
  pick(type) {
    ...
    if (type === 'ASSIST' && this.state.player.physicalState.carrying) {
      this.tool = null;
      this.dispatch({ type: 'ASSIST' });
      return;
    }
    ...
    if (cands.length === 1) {
      this.tool = null;
      this.render();
      this.dispatch({ type, target: cands[0] });
      return;
    }
    if (!cands.length) {
      this.tool = null;
      this.render();
      this.hint(`No ${tool.targets} in range for ${tool.label}`, 'warn');
      return;
    }
    this.tool = type;
    this.render();
  ...
  _candidates(tool) {
    ...
    if (tool.targets === 'door') {
      return s.grid
        .neighbors(p, { vertical: false })
        .map((n) => n.tile)
        .filter(
          (t) =>
            t.type === 'DOOR' &&
            (tool.type !== 'BARRICADE' || (!t.doorState.isOpen && !t.doorState.isBarricaded))
        )
        .map((t) => t.key);
    }
    if (tool.targets === 'debris')
      return s.grid
        .neighbors(p, { vertical: false })
        .map((n) => n.tile)
        .filter((t) => t.debris)
    ...
    if (tool.targets === 'npc') {
      ...
      for (const npc of s.npcs.values()) {
        ...
        if (npc.status !== 'ACTIVE' && !(tool.type === 'ASSIST' && npc.status === 'INCAPACITATED'))
          continue;
  ...
  _onTileClicked({ key, tile }) {
    ...
    if (
      adjacent &&
      tile.type === 'DOOR' &&
      !tile.doorState.isOpen &&
      !tile.doorState.isBarricaded &&
      !tile.doorState.isLocked
    ) {
      this.dispatch({ type: 'TOGGLE_DOOR', target: key });
      return;
    }
    if (adjacent && tile.debris) {
      this.dispatch({ type: 'CLEAR_DEBRIS', target: key });
      return;
    }
    const npc = s.npcAt(key);
    if (npc && tile.type !== 'EXIT') {
      if (adjacent && npc.status === 'INCAPACITATED') {
        this.dispatch({ type: 'ASSIST', target: key });
        return;
      }
  ...
  _onTileHovered({ tile }) {
    if (!tile || this.tool || !this.inputOk() || tile.coord.z !== this.state.player.position.z) {
      this.bus.emit('PATH_PREVIEW', { keys: [] });
      return;
    }
    ...
    return findPath(s.grid, s.player.position, tile.coord, {
      ...
      costFn: (t) => {
        ...
        if (
          t.type === 'DOOR' &&
          !t.doorState.isOpen &&
          (t.doorState.isBarricaded || (t.doorState.isLocked && !master))
        )
          return Infinity;
  ...
  _startPath(tile) {
    const path = this._pathTo(tile);
    if (!path || !path.length) {
      this.hint('No known route there', 'warn');
      return;
    }
  ...
  _advance() {
    ...
    const s = this.state;
    if (!this.inputOk() || s.player.actionPoints.current <= 0) {
      this._clearPath();
      return;
    }
    ...
    if (tile.type === 'DOOR' && !tile.doorState.isOpen) {
      this.dispatch({ type: 'OPEN_DOOR', target: key });
      return;
    }
    this.queue.shift();
  ...
  _onKey(e) {
    ...
    if (k === '[' || k === ']') {
      e.preventDefault();
      this.bus.emit('VIEW_FLOOR_REQUESTED', { delta: k === '[' ? -1 : 1 });
      return;
    }
    if (k === 'u') {
      this.undo();
    ...
    if (k === 'PageUp' || k === 'PageDown') {
      e.preventDefault();
      this._clearPath();
      this.dispatch({ type: 'MOVE', dz: k === 'PageUp' ? 1 : -1 });
      return;
    }
    ...
    if (mv) {
      ...
      this._clearPath();
      this.tool = null;
      this.dispatch({ type: 'MOVE', dx: mv[0], dy: mv[1] });
      return;
    }
  ...
  undo() {
    ...
    if (target < 1) {
      this.hint('Nothing to undo', 'warn');
      return;
    }
    this.tool = null;
    this._clearPath();
