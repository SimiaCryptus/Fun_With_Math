...
export class TurnManager {
  ...
  processPlayerAction(action) {
    const s = this.state;
    if (s.outcome || s.meta.activePhase !== 'PLAYER_INPUT')
      return this._rejected(action, 'Not accepting input right now');
    if (s.player.physicalState.status !== 'ACTIVE')
      return this._rejected(action, 'You are no longer active');
  ...
  _resolveNPC(action, maxDist = 1.5) {
    ...
    if (Grid3D.distance(npc.position, s.player.position) > maxDist && npc.carriedBy !== 'PLAYER')
      return null;
    return npc;
  }
  ...
  _actMOVE(action) {
    ...
    if (!isTilePassable(tile, { canOpenDoors: false })) {
      if (tile.type === 'DOOR' && !tile.doorState.isOpen)
        return { ok: false, reason: 'Door is closed — open it first (1 AP)' };
    ...
    const carrying = s.player.physicalState.carrying
      ? s.npcs.get(s.player.physicalState.carrying)
      : null;
  ...
  _actTOGGLE_DOOR(action) {
    return this._doorAction(action, null);
  }
  _actOPEN_DOOR(action) {
    return this._doorAction(action, true);
  }
  _actCLOSE_DOOR(action) {
    return this._doorAction(action, false);
  }
  ...
  _doorAction(action, desired) {
    ...
    if (Grid3D.distance(tile.coord, s.player.position) > 1.5)
      return { ok: false, reason: 'Door is not adjacent' };
    const ds = tile.doorState;
    const open = desired === null ? !ds.isOpen : desired;
    if (open === ds.isOpen)
      return { ok: false, reason: `Door is already ${open ? 'open' : 'closed'}` };
    if (open && ds.isBarricaded) return { ok: false, reason: 'Door is barricaded' };
    if (open && ds.isLocked && !s.player.inventory.includes('MASTER_KEY'))
      return { ok: false, reason: 'Door is locked' };
    if (!open && (s.npcAt(tile.key) || s.threatAt(tile.key)))
      return { ok: false, reason: 'Someone is standing in the doorway' };
  ...
  _actBARRICADE(action) {
    ...
    if (!s.player.inventory.includes('BARRICADE_KIT'))
      return { ok: false, reason: 'No barricade materials' };
  ...
  _actCLEAR_DEBRIS(action) {
    ...
    if (Grid3D.distance(tile.coord, s.player.position) > 1.5)
      return { ok: false, reason: 'Debris is not adjacent' };
  ...
  _actDEESCALATE(action) {
    ...
    if (
      npc.position.z !== s.player.position.z ||
      !hasLineOfSight(s, s.player.position, npc.position)
    )
      return { ok: false, reason: 'They cannot see you' };
  ...
  commitTurn() {
    ...
    s.log({
      type: 'TURN_SUMMARY',
      fire: s.hazards.fireCells.size,
      smoke: s.hazards.smokeCells.size,
      ...c,
    });
  ...
  _applyExposure() {
    ...
    const apply = (agentPs, key) => {
      ...
      if (D > 0.5) {
        agentPs.lungIrritation += 2;
        agentPs.exposure = 'HEAVY';
      } else if (D >= 0.2) {
        agentPs.lungIrritation += 1;
        agentPs.exposure = 'MILD';
    ...
    if (s.player.physicalState.status === 'ACTIVE' && apply(s.player.physicalState, pKey)) {
      incapacitatePlayer(
        s,
        this.bus,
        s.hazards.fireCells.has(pKey) ? 'thermal exposure' : 'smoke inhalation',
        this._recentFireCauses(s.player.position)
      );
    }
    for (const npc of s.npcs.values()) {
      ...
      if (apply(npc.physicalState, key)) {
        incapacitateNPC(
          ...
          this.bus,
          npc,
          s.hazards.fireCells.has(key) ? 'thermal exposure' : 'smoke inhalation',
          this._recentFireCauses(npc.position)
        );
      }
    }
  }
  ...
  _evaluateOutcome() {
    ...
    if (ps !== 'ACTIVE')
      return {
        result: 'LOSS',
        reason: `You were ${ps.toLowerCase()}. The scenario cannot continue.`,
        counts: c,
      };
    for (const cond of conds) {
      if (cond.type === 'PREVENT_CASUALTIES' && c.casualties > cond.targetValue) {
        return {
          result: 'LOSS',
          reason: `Casualty threshold exceeded (${c.casualties} > ${cond.targetValue}).`,
          counts: c,
        };
      }
    }
    const final =
      s.social.responderEtaTurns <= 0 || c.active === 0 || s.meta.turnNumber >= s.meta.maxTurns;
    const checks = conds
      .filter((k) => k.type !== 'PREVENT_CASUALTIES')
      .map((cond) => {
        let met = false;
        if (cond.type === 'EVACUATE_MINIMUM_PERCENT')
          met = c.total === 0 || c.evacuated / c.total >= cond.targetValue;
        else if (cond.type === 'SURVIVE_TURNS') met = s.meta.turnNumber >= cond.targetValue;
        else if (cond.type === 'CONTAIN_HAZARD')
          met =
            s.hazards.fireCells.size <= cond.targetValue &&
            (final || s.hazards.fireCells.size === 0);
        return { cond, met };
      });
    ...
    if (allMet && (final || earlyOk)) {
      return {
        result: 'WIN',
        reason: final ? 'Responders arrived with objectives met.' : 'Evacuation objective reached.',
        counts: c,
      };
    }
    if (final) {
      const unmet = checks
        .filter((k) => !k.met)
        .map((k) => `${k.cond.type} (${k.cond.targetValue})`)
        .join(', ');
      return {
        result: 'LOSS',
        reason: `Responders arrived but objectives were unmet: ${unmet || 'none remaining active'}.`,
        counts: c,
      };
    }
    return null;
