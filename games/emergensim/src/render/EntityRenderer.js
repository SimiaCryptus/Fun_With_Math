...
export class EntityRenderer {
  ...
  _label(tok, text, color = '#f3f4f6') {
    ...
    if (tok.label) {
      tok.group.remove(tok.label);
      disposeObject(tok.label);
    }
  ...
  sync(viewFloor, revealFn) {
          ...
          : ps.exposure === 'HEAVY'
            ...
            : ps.slowed
              ...
              : ps.exposure === 'MILD'
                ...
                : '';
      this._style(tok, {
        down: ps.status !== 'ACTIVE',
        ring:
          ps.status !== 'ACTIVE' ? RING.INCAP : carrying ? RING.CARRY : RING[ps.exposure] || null,
      });
      this._label(
        tok,
        `You${badge ? ' · ' + badge : ''}${carrying ? ' · carrying ' + carrying.name : ''}`,
        '#38bdf8'
      );
    }
    ...
    for (const npc of s.npcs.values()) {
      ...
      this._style(tok, {
        down: inc,
        ring: inc ? RING.INCAP : npc.freezeTurns > 0 ? RING.FROZEN : RING[nps.exposure] || null,
      });
  ...
  update(dt) {
    ...
    for (const tok of this.tokens.values()) {
      ...
      if (tok.snap || pos.distanceToSquared(tok.target) > 36) {
        pos.copy(tok.target);
        tok.snap = false;
  ...
  dispose() {
    ...
    this.tokens.clear();
    for (const g of [this.bodyGeo, this.headGeo, this.ringGeo, this.coneGeo, this.visionGeo])
      g.dispose();
  }
}
