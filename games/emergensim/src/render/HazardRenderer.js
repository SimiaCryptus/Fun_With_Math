...
export class HazardRenderer {
  ...
  sync(viewFloor, revealFn, exploredFn) {
    ...
    for (const [key, m] of this.fire)
      if (!liveFire.has(key)) {
        this.scene.remove(m);
        this.fire.delete(key);
      }
    if (n) {
      this.light.position.set(cx / n, viewFloor * FLOOR_H + 1.6, cz / n);
      this.light.intensity = 10 + n * 3;
    ...
    for (const [key, m] of this.smoke)
      if (!liveSmoke.has(key)) {
        this.scene.remove(m);
        m.material.dispose();
        this.smoke.delete(key);
      }
    ...
    for (const [key, m] of this.debris)
      if (!liveDebris.has(key)) {
        this.scene.remove(m);
  ...
  dispose() {
    ...
    for (const m of this.smoke.values()) {
      this.scene.remove(m);
      m.material.dispose();
    }
