...
export class ThreatEntityModel {
  ...
  _agents(z) {
    ...
    if (p.physicalState.status === 'ACTIVE' && p.position.z === z)
      out.push({ id: 'PLAYER', position: p.position });
    for (const npc of this.state.npcs.values()) {
      if (npc.status === 'ACTIVE' && !npc.carriedBy && npc.position.z === z)
        out.push({ id: npc.id, position: npc.position });
    }
    return out;
  }

  _inCone(t, pos) {
    const vx = pos.x - t.position.x,
      vy = pos.y - t.position.y;
  ...
  _perceive(t) {
    const seen = this._agents(t.position.z)
      .filter(
        (a) =>
          Grid3D.distance(t.position, a.position) <= VISION_RANGE &&
          this._inCone(t, a.position) &&
          hasLineOfSight(this.state, t.position, a.position, VISION_RANGE)
      )
      .sort(
        (a, b) => Grid3D.distance(t.position, a.position) - Grid3D.distance(t.position, b.position)
      );
    if (seen.length) {
      const prev = t.state;
      t.state = 'PURSUIT';
      t.targetId = seen[0].id;
      t.lastSeen = { ...seen[0].position };
      t.lostTurns = 0;
      if (prev !== 'PURSUIT')
        this.state.log({
          type: 'THREAT_STATE',
    ...
    if (t.state === 'PURSUIT') {
      ...
      if (t.lostTurns > 2) {
        t.state = 'INVESTIGATE';
        t.investigateTarget = t.lastSeen;
        t.targetId = null;
      }
      return;
    }
    const heard = this.state.social.noiseEvents
      .filter(
        (n) =>
          n.position.z === t.position.z && Grid3D.distance(t.position, n.position) <= n.loudness + 2
      )
      .sort(
        (a, b) => Grid3D.distance(t.position, a.position) - Grid3D.distance(t.position, b.position)
      );
    if (heard.length) {
      t.state = 'INVESTIGATE';
      t.investigateTarget = { ...heard[0].position };
      this.state.log({
        type: 'THREAT_STATE',
        threatId: t.id,
        state: 'INVESTIGATE',
        key: keyOf(t.investigateTarget),
        source: heard[0].source,
      });
    }
  }
  ...
  _goal(t) {
    ...
    if (t.state === 'INVESTIGATE') {
      if (t.investigateTarget && keyOf(t.investigateTarget) !== keyOf(t.position))
        return { goal: t.investigateTarget, steps: 2 };
      t.state = 'PATROL';
      t.investigateTarget = null;
    }
    if (t.patrolRoute.length) {
      let wp = t.patrolRoute[t.patrolIndex % t.patrolRoute.length];
      if (keyOf(wp) === keyOf(t.position)) {
        t.patrolIndex = (t.patrolIndex + 1) % t.patrolRoute.length;
        wp = t.patrolRoute[t.patrolIndex];
      }
  ...
  _act(t) {
    ...
    const path = findPath(s.grid, t.position, plan.goal, {
      passable: (tile) =>
        ...
        tile.type !== 'WALL' &&
        tile.type !== 'WINDOW' &&
        !s.hazards.fireCells.has(tile.key),
    });
    if (!path) return;
    for (let i = 0; i < plan.steps && i < path.length; i++) {
      ...
      t.facing = {
        dx: Math.sign(tile.coord.x - t.position.x),
        dy: Math.sign(tile.coord.y - t.position.y),
      };
  ...
  _force(t, tile) {
    ...
    if (ds.barricadeStrength <= 0) {
      ds.barricadeStrength = 0;
      ds.isLocked = false;
      ds.isBarricaded = false;
      ds.isOpen = true;
      ...
      this.state.log({
        type: 'BARRICADE_STRESSED',
        key: tile.key,
        remaining: ds.barricadeStrength,
        threatId: t.id,
      });
