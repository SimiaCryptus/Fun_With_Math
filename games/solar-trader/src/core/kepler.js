...
export function propagate(r0, v0, dt, mu = 1) {
  ...
  if (alpha > 1e-9) {
    ...
    chi =
      s *
      Math.sqrt(-a) *
      Math.log((-2 * mu * alpha * dt) / (rdv + s * Math.sqrt(-mu * a) * (1 - r0m * alpha)));
  } else {
    // near-parabolic
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const dtn =
      (chi * chi * chi * c3 + (rdv / sq) * chi * chi * c2 + r0m * chi * (1 - psi * c3)) / sq;
...
export function stateToElements(r, v, mu = 1) {
  ...
  const nm = norm(nvec);
  const evec = scale(sub(scale(r, vm * vm - mu / rm), scale(v, dot(r, v))), 1 / mu);
  const e = norm(evec);
