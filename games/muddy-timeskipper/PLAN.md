...
- Snapback cannot _increase_ forward speed by more than `SNAP_GAIN_CAP = 1.15x` current
...
- HUD **VELOCITY RIBBON**: scrolling strip chart of the last 40 s of player speed with
  tick markers at −8 s / −20 s / −40 s and a ghost pip showing _the speed you would snap
  to_ for the currently selected gear. This single widget converts the mechanic from
...
- `VehicleRig` (three.js): body group with non-uniform scale in _velocity-aligned_ local
...
- Accept: snapback reads instantly as a _joke_ on screen even with the HUD hidden;
