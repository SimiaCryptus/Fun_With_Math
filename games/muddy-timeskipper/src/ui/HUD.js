...
export class HUD {
  ...
  _speedo(g, mph, snap) {
    ...
    g.strokeText('MPH', x + 150, y + 88);
    g.fillText('MPH', x + 150, y + 88);
    if (snap.armed) {
  ...
  _thermometer(g, strain, armed) {
    ...
    grad.addColorStop(0, '#7ab317');
    grad.addColorStop(0.6, '#ffd21f');
    grad.addColorStop(1, '#ff3b1f');
    g.fillStyle = grad;
    g.beginPath();
    g.roundRect(x + 3, y + h - fh + 3, w - 6, Math.max(0, fh - 6), 9);
    g.fill();
    if (armed) {
      g.strokeStyle = '#c8ff2a';
      g.lineWidth = 3;
      g.stroke();
    }
    g.restore();
  ...
  _ribbon(g, race) {
    ...
    const W = Math.min(620, this.w - 300),
      H = 96;
    const x = 24,
      y = this.h - H - 24;
    ...
    g.fillText(
      `${gear.name.toUpperCase()}  →  ${msToMph(s.speed).toFixed(0)} MPH${s.clamped ? '  (TOO YOUNG!!)' : ''}`,
      x + 8,
      y - 8
    );

    // cooldown bar
    const cd = race.snapback.cooldown,
      cdMax = gear.cooldown;
    if (cd > 0) {
  ...
  _stamp(g, dt) {
    ...
    g.lineWidth = 10;
    g.fillStyle = '#ffd21f';
    g.strokeText(this.stamp.text, 0, 0);
    g.fillText(this.stamp.text, 0, 0);
    g.restore();
  }

  _countdown(g, t) {
    g.save();
    g.font = '700 160px Impact, sans-serif';
    g.textAlign = 'center';
    g.strokeStyle = '#000';
    g.lineWidth = 12;
    g.fillStyle = '#7ab317';
    ...
    g.strokeText(txt, this.w / 2, this.h / 2);
    g.fillText(txt, this.w / 2, this.h / 2);
    g.restore();
  }
}
