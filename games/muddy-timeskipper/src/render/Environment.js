...
function bannerTexture(text) {
  ...
  g.font = '700 140px Impact, sans-serif';
  g.textAlign = 'center';
...
export class Environment {
  ...
  _hills(field) {
    ...
    const cx = field.minX + field.size / 2,
      cz = field.minZ + field.size / 2;
    for (let i = 0; i < 16; i++) {
      const a = (i / 16) * TAU + rng.range(-0.15, 0.15);
      const r = rng.range(760, 1080);
      const w = rng.range(180, 440),
        h = rng.range(55, 160);
      const m = new THREE.Mesh(geo, mat);
  ...
  _barriers(spline) {
    ...
    const stack = mergeGeometries(
      [0, 1, 2].map((k) => tire.clone().translate(0, 0.24 + k * 0.46, 0))
    );
    tire.dispose();

    const N = spline.samples,
      step = 4.6;
    ...
    const black = new THREE.Color(0x1c1917),
      cream = new THREE.Color(0xd8cfbb),
      red = new THREE.Color(0xb8321f);
    let i = 0;
    for (let n = 0; n < per; n++) {
  ...
  _gantry(spline, track) {
    ...
    const beam = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        map: bannerTexture(track.name?.toUpperCase() || 'START'),
        roughness: 0.8,
      })
    );
    beam.position.y = 7.4;
  ...
  _checkpoints(spline, track) {
    ...
    for (let c = 1; c < cps.length; c++) {
      ...
      for (const side of [-1, 1]) {
        ...
        pole.position.set(x, terrainNoise(x, z), z);
  ...
  _scatter(spline, field, count, clearance, place) {
    const rng = this.rng;
    const x0 = field.minX - 140,
      x1 = field.minX + field.size + 140;
    const z0 = field.minZ - 140,
      z1 = field.minZ + field.size + 140;
    let placed = 0;
  ...
  _flora(spline, field, track) {
    ...
    for (const [a, h] of [
      ...
      parts.push(
        new THREE.CylinderGeometry(0.05, 0.2, 3.4, 5)
          .translate(0, 1.7, 0)
          .rotateZ(0.8)
          .rotateY(a)
          .translate(0, h, 0)
    ...
    const rocks = new THREE.InstancedMesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x5f5648, roughness: 1, flatShading: true }),
      90
    );
    ...
    const reeds = new THREE.InstancedMesh(
      new THREE.ConeGeometry(0.14, 2.4, 5).translate(0, 1.2, 0),
      new THREE.MeshStandardMaterial({ color: 0x6b7d2c, roughness: 1 }),
      zones.length * 60
    );
    let r = 0;
    for (const zn of zones) {
      for (let tries = 0; tries < 400 && r < (zones.indexOf(zn) + 1) * 60; tries++) {
        const a = rng.range(0, TAU),
          d = zn.r * rng.range(0.7, 1.4);
        const x = zn.x + Math.cos(a) * d,
          z = zn.z + Math.sin(a) * d;
        const n = spline.nearest(x, z, -1);
