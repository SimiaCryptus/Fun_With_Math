...
export const ENTRY_KEY_ORDER = [
    'id', 'category', 'section', 'order', 'icon', 'title', 'subtitle',
    'href', 'readme', 'video', 'launchLabel', 'pitch', 'tags', 'hidden', 'repo',
];

export const REPO_KEY_ORDER = [
    'url', 'remote', 'host', 'slug', 'path', 'subpath', 'commit', 'branch', 'submodule',
];
...
function emptyManifest() {
    return {version: MANIFEST_VERSION, generatedAt: new Date().toISOString(), counts: {}, entries: []};
}
...
const ENTRY_FIELDS = [
    {path: 'id', label: 'id', kind: 'text', required: true, hint: 'Stable slug, unique across the site'},
    ...
    {path: 'order', label: 'order', kind: 'number', hint: 'Sort key within the section; missing sorts last'},
    ...
    {path: 'pitch', label: 'pitch', kind: 'textarea', span: true, hint: 'Short blurb; inline HTML allowed'},
    {path: 'tags', label: 'tags', kind: 'tags', hint: 'Comma-separated'},
    {path: 'hidden', label: 'hidden', kind: 'checkbox', hint: 'Excluded from the published manifest'},
];
...
const META_FIELDS = [
    ...
    {path: 'source', label: 'source', kind: 'text', hint: `Root-relative path of the ${ENTRY_FILENAME} sidecar`},
];
...
function buildForm() {
    dom.form.innerHTML = [
        ...
        fieldsetHtml('Repository', REPO_FIELDS, '<p class="hint"><a id="repo-browse" href="#" target="_blank" rel="noopener" hidden></a></p>'),
...
function renderList() {
    ...
    const rows = visibleEntries().map(({entry, index}) => {
        ...
        const order = Number.isFinite(entry.order)
            ? (entry.order === Number.MAX_SAFE_INTEGER ? '∞' : entry.order)
            : '—';
...
function problemsByEntry() {
    ...
    state.manifest.entries.forEach((entry, i) => {
        ...
        if (entry.id) {
            if (byId.has(entry.id)) msgs.push(`${label}: duplicate id (also at entries[${byId.get(entry.id)}])`);
...
async function loadFromUrl(url, {silent = false} = {}) {
    try {
        ...
        const hint = location.protocol === 'file:' ? ' (fetch is blocked on file:// — use Import…)' : '';
...
function newEntry() {
    const category = state.filter.category !== 'all' ? state.filter.category : (state.selected?.category ?? 'game');
    ...
    const at = state.selected ? state.manifest.entries.indexOf(state.selected) + 1 : state.manifest.entries.length;
...
function syncPaths() {
    const entry = state.selected;
    if (!entry) return;
    const dir = normalizePosix(joinPosix(CATEGORY_ROOTS[entry.category] ?? '', slugify(entry.id || entry.title)));
    entry.dir = dir;
...
function wire() {
    ...
    dom.filterCategory.innerHTML =
        ['all', ...CATEGORIES].map((c) => `<option value="${c}">${c}</option>`).join('');
