...
import { ENTRY_FILENAME, ENTRY_KEY_ORDER, LEGACY_SOURCES, entryDirFor, entryIdFromDir, isExternal, isMetaKey, joinPosix, orderKeys, serializeJson, splitPathSuffix, toPathRef, uniqueId, validateEntryFile, } from '../manifest/schema.ts';
/* ---------------------------------------------------------------- args */
...
async function collect() {
    ...
    for (const source of LEGACY_SOURCES) {
        const doc = (await readJson(source.file));
        if (!doc) {
        ...
        for (const section of sections) {
            ...
            for (let index = 0; index < bucket.length; index += 1) {
                ...
                if (id !== base)
                    warnings.push(`id collision on "${base}" → using "${id}" for ${dir}`);
                ...
                const refs = [href, readme, video].filter((v) => Boolean(v));
...
async function main() {
    ...
    for (const { dir, entry, refs } of pending) {
        ...
        for (const ref of refs) {
            ...
            if (!ok)
                warnings.push(`missing file referenced by ${entry.id}: ${clean}`);
        }
...
main().catch((err) => {
    console.error(`split-manifest failed: ${err.message}`);
    process.exitCode = 1;
});
