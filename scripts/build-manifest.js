...
async function* findEntryFiles(relDir, depth = 0) {
...
function repoForDir(dir, ctx, source, warnings, warned) {
  const match = matchRepoPath(
    dir,
    ctx.submodules.map((s) => s.path)
  );
  if (match) {
    ...
    if (sub.state === '-' && !warned.has(sub.path)) {
      warned.add(sub.path);
      warnings.push(
        `${source}: submodule "${sub.path}" is not initialized — commit pin may be stale`
      );
    }
...
async function loadEntries(gitContext) {
  const entries = [];
  const errors = [];
  const warnings = [];
  ...
  for (const source of sources) {
    ...
    try {
      ...
      errors.push(`${source}: invalid JSON — ${err.message}`);
      continue;
    }
    ...
    if (VERIFY) {
      ...
      for (const [field, value] of Object.entries({
        ...
        if (!(await exists(clean)))
          warnings.push(`${source}: ${field} points at missing file "${clean}"`);
      }
    }
    entries.push(entry);
  }
...
async function main() {
  ...
  if (previous) {
    try {
      ...
      const sameBody =
        JSON.stringify({ ...old, generatedAt: '' }) ===
