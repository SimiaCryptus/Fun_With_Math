...
el.rollBtn.addEventListener("click", () => {
  ...
  if (!Number.isInteger(die) || die < 1 || die > 20) {
    ...
    el.rollResult.innerHTML = `<div class="roll-outcome">🎲 Roll the foam die, then type a number from 1 to 20!</div>`;
    el.rollValue.focus();
    return;
  }
  ...
  addLog(
    `🎲 ${hero.name} rolled ${total} (${statId}) vs TN ${tn} — ${
      die === 20
        ? "natural 20!"
        : die === 1
          ? "natural 1!"
...
function renderChoice(choice) {
  ...
  const opts = choice.options
    .map(
      (o, i) =>
        `<li><b>${String.fromCharCode(65 + i)}.</b> ${escapeHtml(o)}</li>`,
    )
    .join("");
...
function loadGame(showMsg) {
  try {
    ...
    if (!raw) {
      if (showMsg) flashStatus("No saved game found.");
      return;
    }
    const data = JSON.parse(raw);
    state.heroes = Array.isArray(data.heroes) ? data.heroes : [];
    state.heroes.forEach((h) => {
      if (typeof h.pos !== "number") h.pos = 0;
    });
