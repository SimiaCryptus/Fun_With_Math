...
function renderDocMeta(label) {
  ...
  $('#doc-meta').replaceChildren(
    ...
    state.misplaced.length
      ? el(
          'span',
          { class: 'sev-warn' },
          ` · ${state.misplaced.length} link(s) recovered from nodes[]`
        )
      : null
  );
}
...
function buildFilterChips() {
  ...
  mk(
    '#f-kinds',
    [...kindCount].sort().map(([k, c]) => [k, c, S.LAYER_COLOR[S.NODE_KIND_LAYER[k]]]),
    f.kinds
  );
  mk(
    '#f-status',
    [...statusCount].sort().map(([k, c]) => [k, c, null]),
    f.statuses
  );
  mk(
    '#f-relations',
    [...relCount].sort().map(([k, c]) => [k, c, null]),
    f.relations
  );
  ...
  $('#legend').replaceChildren(
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#7f8ea3' }),
      'logical / structural'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#fbbf24' }),
      'evidential'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#ff6b6b' }),
      'conflict'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#a78bfa' }),
      'cross-layer'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#2dd4bf' }),
      'contextual'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#38bdf8;border-top-style:dashed' }),
      'morphism'
    ),
    el(
      'div',
      { class: 'row' },
      el('span', { class: 'sw', style: 'border-color:#f472b6;border-top-style:dotted' }),
      'coherence'
    )
  );
}
  ...
  if (f.depth > 0 && state.selection && state.index.get(state.selection)?.type === 'node') {
    ...
    for (let d = 0; d < f.depth; d++) {
      ...
      for (const id of frontier) {
        ...
        for (const m of inc.morph)
          for (const o of [m.from, m.to])
            if (!reach.has(o)) {
              reach.add(o);
              next.push(o);
            }
...
function badge(text, style) {
  return el('span', { class: 'badge' + (style ? ' ' + style.cls : ''), style: style?.css }, text);
}
    ...
    ...sources.map((s) =>
      el(
        ...
        el(
          'div',
          { class: 'loc' },
          [s.file, s.heading, s.lines ? `L${s.lines[0]}–${s.lines[1]}` : null]
            .filter(Boolean)
            .join(' › ')
        ),
...
function renderInspector(id) {
  ...
  if (!id || !state.index.has(id)) {
    host.append(
      el('div', { class: 'placeholder' }, 'Select a node, edge, morphism or obligation.')
    );
    return;
  }
...
function renderNodeInspector(host, n) {
          ...
          ...r.interfaces.map((i) =>
            el(
              ...
              el(
                'span',
                { class: 'rel-tag' },
                `→${i.to}${i.via ? ' via ' + i.via : ''}${i.available ? '' : ' (unavailable)'}${i.lossy ? ' lossy' : ''}`
              ),
  ...
  if (n.shape) {
    ...
    host.append(
      kv([
        ['descriptor', n.shape.descriptor],
        ['dims', (n.shape.dims || []).join(', ')],
      ])
    );
  }
        ...
        ...n.similar_to.map((s) =>
          el(
            ...
            el(
              'span',
              { class: 'rel-tag' },
              `${s.score?.toFixed?.(2) ?? s.score}${s.basis ? ' ' + s.basis : ''}`
            ),
            link(s.to)
          )
        )
  ...
  if (inc.out.length || inc.in.length) {
    ...
    host.append(
      el(
        ...
        { class: 'plain' },
        ...inc.out.map((e) =>
          el('li', {}, el('span', { class: 'rel-tag' }, '→ ' + e.relation), link(e.to))
        ),
        ...inc.in.map((e) =>
          el('li', {}, el('span', { class: 'rel-tag' }, '← ' + e.relation), link(e.from))
        )
      )
    );
  }
  ...
  if (inc.coh.length) {
    ...
    host.append(
      el(
        ...
        { class: 'plain' },
        ...inc.coh.map((c) =>
          el('li', {}, el('span', { class: 'rel-tag' }, `${c.kind}/${c.status}`), link(c.id, c.id))
        )
      )
    );
  }
...
function renderEdgeInspector(host, e) {
  host.append(
    ...
    el(
      ...
      { class: 'badges' },
      badge('edge'),
      badge(S.RELATION_CLASS[e.relation] || '?'),
  ...
  host.append(
    ...
    kv([
      ...
      [
        'cost',
        m.cost
          ? `${m.cost.measure}${m.cost.value != null ? ' ' + m.cost.value : ''}${m.cost.unit ? ' ' + m.cost.unit : ''}${m.cost.note ? ' — ' + m.cost.note : ''}`
          : null,
      ],
      [
        'obligations',
        (m.obligations || []).length
          ? el('span', {}, ...(m.obligations || []).map((o) => [link(o, o), ' ']))
          : null,
      ],
      ['notes', m.notes],
    ])
          ...
          [
            {
              label: 'layer',
              get: (r) =>
                el('span', { class: 'pill', style: `color:${S.LAYER_COLOR[r._layer]}` }, r._layer),
            },
    ...
    case 'morphisms': {
      ...
      body.append(
        rows.length
          ...
          : el('div', { class: 'empty-note' }, 'No morphisms in this graph.')
      );
      break;
    }
    case 'coherence': {
      ...
      body.append(
        rows.length
          ...
          : el('div', { class: 'empty-note' }, 'No coherence obligations.')
      );
      break;
    }
    case 'clusters': {
      ...
      body.append(
        rows.length
          ...
          : el('div', { class: 'empty-note' }, 'No clusters.')
      );
      break;
    }
    case 'issues': {
...
function exportVisible() {
  ...
  const out = {
    ...
    clusters: (g.clusters || [])
      .map((c) => ({ ...c, members: (c.members || []).filter((m) => vis.has(m)) }))
