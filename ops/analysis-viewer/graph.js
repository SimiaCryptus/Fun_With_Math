...
function shapePath(layer, r) {
  switch (layer) {
    ...
    case "fuzzy": { // circle
      const k = r * 0.5523;
      return `M ${-r} 0 C ${-r} ${-k} ${-k} ${-r} 0 ${-r} C ${k} ${-r} ${r} ${-k} ${r} 0`
        + ` C ${r} ${k} ${k} ${r} 0 ${r} C ${-k} ${r} ${-r} ${k} ${-r} 0 Z`;
    }
    case "symbolic": { // rounded square
      const a = r * 0.95, c = r * 0.45;
      return `M ${-a + c} ${-a} H ${a - c} Q ${a} ${-a} ${a} ${-a + c} V ${a - c}`
        + ` Q ${a} ${a} ${a - c} ${a} H ${-a + c} Q ${-a} ${a} ${-a} ${a - c} V ${-a + c}`
        + ` Q ${-a} ${-a} ${-a + c} ${-a} Z`;
    }
    case "deductive": { // square
      const a = r * 0.92;
      return `M ${-a} ${-a} H ${a} V ${a} H ${-a} Z`;
    }
    case "numeric": { // diamond
      const a = r * 1.2;
      return `M 0 ${-a} L ${a} 0 L 0 ${a} L ${-a} 0 Z`;
    }
    default: { // hexagon (social / ecological)
      const a = r * 1.05, h = a * 0.87;
...
export class GraphView {
  ...
  _buildSvg() {
    ...
    for (const [cls, color] of Object.entries({
      "rel-logic": "#7f8ea3", "rel-evidence": "#fbbf24", "rel-conflict": "#ff6b6b",
      "rel-cross": "#a78bfa", "rel-context": "#2dd4bf",
      morphism: "#38bdf8", coherence: "#f472b6",
    })) {
      const m = svgEl("marker", {
        id: `arrow-${cls}`, viewBox: "0 0 10 10", refX: 9, refY: 5,
  ...
  _wirePanZoom() {
    ...
    this.svg.addEventListener("pointerdown", (e) => {
      ...
      ox = this.transform.x; oy = this.transform.y;
  ...
  setData(graph) {
    ...
    for (const n of graph.nodes) {
      ...
      this.nodes.set(n.id, {
        id: n.id, data: n, lane,
        x: this.lanes.indexOf(lane) * LANE_W, y: 0,
        pinned: false,
      });
    }
  ...
  layout() {
    ...
    for (const [, arr] of byLane) {
      ...
      arr.forEach((n, i) => { n.y = (i - (arr.length - 1) / 2) * ROW_H; });
    }

    const nbrs = new Map([...this.nodes.keys()].map((id) => [id, []]));
    for (const l of this.allLinks) { nbrs.get(l.from).push(l.to); nbrs.get(l.to).push(l.from); }
    ...
    for (let pass = 0; pass < 40; pass++) {
      ...
      for (const lane of order) {
        ...
        for (const n of arr) {
          const ys = nbrs.get(n.id).map((id) => this.nodes.get(id)).filter((m) => m && m.lane !== lane).map((m) => m.y);
  ...
  render(visibleNodeIds, links, matchIds = new Set()) {
    ...
    for (const lane of this.lanes) {
      ...
      const g = svgEl("g");
      g.append(svgEl("rect", {
        class: "lane-rect", x: x - 30, y: minY - 56,
        width: LANE_W - 30, height: (maxY - minY) + 100, rx: 8,
      }));
      const lbl = svgEl("text", { class: "lane-label", x: x - 18, y: minY - 34, fill: LAYER_COLOR[lane] });
    ...
    for (const l of links) {
      const a = this.nodes.get(l.from), b = this.nodes.get(l.to);
      ...
      p.dataset.to = l.to;
      p.addEventListener("click", (e) => { e.stopPropagation(); this.onSelect(l.id, l.kind); });
    ...
    for (const n of visNodes) {
      ...
      g.append(svgEl("path", {
        class: "shape", d: shapePath(n.lane, r),
      ...
      g.addEventListener("click", (e) => { e.stopPropagation(); this.onSelect(n.id, "node"); });
  ...
  _makeDraggable(g, n) {
    ...
    g.addEventListener("pointermove", (e) => {
      ...
      n.x = ox + dx; n.y = oy + dy; n.pinned = true;
  ...
  updatePositions() {
    ...
    for (const [id, p] of this.linkEls || []) {
      const a = this.nodes.get(p.dataset.from), b = this.nodes.get(p.dataset.to);
  ...
  _linkPath(a, b) {
    ...
    const ax = a.x + sign * 12, bx = b.x - sign * 14;
    const dx = (bx - ax) * 0.45;
  ...
  highlight(id) {
    ...
    if (id && this.nodes.has(id)) {
      ...
      for (const l of this.links) {
        if (l.from === id || l.to === id) { linksOn.add(l.id); nodesOn.add(l.from); nodesOn.add(l.to); }
      }
    }
  ...
  fit(padding = 60) {
    ...
    const minX = Math.min(...xs) - 60, maxX = Math.max(...xs) + 230;
    const minY = Math.min(...ys) - 80, maxY = Math.max(...ys) + 40;
    const r = this.svg.getBoundingClientRect();
    const k = Math.min(
    ...
    this.transform.x = padding - minX * this.transform.k
      + (r.width - padding * 2 - (maxX - minX) * this.transform.k) / 2;
    this.transform.y = padding - minY * this.transform.k
