/**
 * schema.js — runtime mirror of ops/theory_graph.schema.ts
...
export const LANE_ORDER = ["social", "inspiration", "fuzzy", "symbolic", "deductive", "numeric", "ecological"];

export const LAYER_INDEX = {
  inspiration: -2, fuzzy: 0, symbolic: 1, deductive: 2, numeric: 3,
  social: null, ecological: null,
};

export const LAYER_DUAL = {
  inspiration: "social", social: "inspiration",
  numeric: "ecological", ecological: "numeric",
};
...
export const LAYER_SEMANTICS = {
  inspiration: { identity: "a directional inference about the space of possible theories", truth: null },
  ...
  numeric: { identity: "a limit of computable approximations", truth: "stability under refinement" },
  social: { identity: "a distributed epistemic fact", truth: "community acceptance" },
  ecological: { identity: "a condition of the working environment", truth: "survival of contact with real constraints" },
};

export const NODE_KIND_LAYER = {
  trajectory: "inspiration", analogy: "inspiration", domain_mapping: "inspiration",
  invariance_hypothesis: "inspiration", knowledge_gap: "inspiration",

  proto_pattern: "fuzzy", proto_operator: "fuzzy", sketch: "fuzzy", metaphor: "fuzzy",
  conjecture: "fuzzy", heuristic: "fuzzy", open_question: "fuzzy",

  definition: "symbolic", notation: "symbolic", rewrite_rule: "symbolic",
  signature: "symbolic", model: "symbolic",

  axiom: "deductive", theorem: "deductive", lemma: "deductive", proof: "deductive", theory: "deductive",

  observation: "numeric", experiment: "numeric", bound: "numeric", certificate: "numeric",

  reference: "social", norm: "social", research_program: "social", folklore: "social",

  constraint: "ecological", resource: "ecological", artifact: "ecological",
};
...
export const RELATION_CLASS = (() => {
  ...
  put("rel-logic", "assumes", "depends_on", "implies", "generalizes", "specializes",
    "equivalent_to", "refines", "instantiates", "cites");
...
export const COHERENCE_LAW = {
  semantic: { between: ["numeric", "symbolic"], requirement: "numeric evaluation must respect symbolic identities" },
  logical: { between: ["symbolic", "deductive"], requirement: "symbolic rewrites must preserve provable truths" },
  analytic: { between: ["deductive", "numeric"], requirement: "proofs must guarantee the convergence properties they claim" },
};
...
export function descentDirection(from, to) {
  const a = LAYER_INDEX[from], b = LAYER_INDEX[to];
...
export function validateTheoryGraph(g) {
  ...
  for (const n of g.nodes) {
    ...
    if (n.layer !== undefined && !LAYERS.includes(n.layer)) problems.push(`bad layer on ${n.id}: ${n.layer}`);
    ...
    if (LAYER_SEMANTICS[layer] && LAYER_SEMANTICS[layer].truth === null &&
  ...
  for (const e of g.edges || []) {
    ...
    edgeIds.add(e.id);
    if (!RELATION_KINDS.includes(e.relation)) problems.push(`bad relation on ${e.id}: ${e.relation}`);
    ...
    const lf = layerById.get(e.from), lt = layerById.get(e.to);
    if (rule && lf && lt) {
  ...
  for (const m of g.morphisms || []) {
    ...
    morphIds.add(m.id);
    if (!MORPHISM_KINDS.includes(m.kind)) { problems.push(`bad morphism kind on ${m.id}: ${m.kind}`); continue; }
    ...
    const lf = layerById.get(m.from), lt = layerById.get(m.to);
...
export function computeStats(g) {
  const by_kind = {}, by_layer = {}, by_relation = {}, by_morphism = {}, by_coherence = {};
  const layerById = new Map();
  for (const n of g.nodes || []) {
  ...
  return {
    ...
    by_kind, by_relation, by_layer, by_morphism, by_coherence,
...
export function normalizeGraph(raw) {
  const g = {
    ...raw,
    nodes: [], edges: [...(raw.edges || [])], morphisms: [...(raw.morphisms || [])],
    coherence: [...(raw.coherence || [])], clusters: [...(raw.clusters || [])],
